...
def flow_cluster_DBSCAN_with_scale_factor(OD, scale_factor, min_flows, n_jobs=None, flow_distance_func: Callable = flow_distance_matrix_max_euclidean):
    ...
    assert n == m, f"distance_matrix must be a square matrix."
    assert n == len(eps_array), f"distance_matrix.shape[0] must be equal to len(eps_array)."

    labels = np.full(n, -1)  # 初始化所有点的标签为 -1，表示未分类
    ...
    def process_point(point_idx):
        if labels[point_idx] != -1:
            return None  # 如果点已经被分类，跳过
        neighbors = region_query(point_idx)
        if len(neighbors) < minpts:
            labels[point_idx] = -2  # 标记为噪声
            return None
        else:
